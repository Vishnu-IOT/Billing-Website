...
export default function B2BInvoice({
  ...
  if (!isComposition) {
    items.forEach((item) => {
      ...
      const taxable = parseFloat(item.baseRate ?? item.afterDiscount ?? 0) || 0;

      const taxAmt = parseFloat(item.taxAmount || 0) || 0;
  ...
  const gstRows = Object.values(gstSummary).sort((a, b) => a.rate - b.rate);

  const itemsSubtotal = items.reduce(
    (sum, item) =>
      sum + (parseFloat(item.baseRate ?? item.afterDiscount ?? 0) || 0),
    0,
  );
  const extraDiscount =
    parseFloat(
  ...
  const companyGstin =
    financials.gstin || companies.companyGstin || companies.gstin || "";
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span
          style={{ fontWeight: 800, fontSize: 16, textTransform: "uppercase" }}
        >
          {docTitle}
        </span>
      ...
      >
        ...
        <div style={{ flex: 1, textAlign: "center" }}>
          ...
          <div style={{ fontSize: 12.5, color: "#333" }}>
            {(companies.companyPhone || companies.phone) &&
              `Mobile: +91 ${companies.companyPhone || companies.phone}`}
            {(companies.companyPhone || companies.phone) &&
              (companies.email || companies.companyEmail) &&
              "  |  "}
            {(companies.email || companies.companyEmail) &&
              `Email: ${companies.email || companies.companyEmail}`}
          </div>
        </div>
      </div>
      ...
      <div style={{ display: "flex", borderBottom: "1px solid #999" }}>
        ...
        >
          ...
          {party.address && (
            <div style={{ fontSize: 12.5, marginTop: 4 }}>{party.address}</div>
          )}
        </div>
        <div
          style={{
          ...
          {items.map((item, idx) => {
            ...
            return (
              <tr key={idx}>
                ...
                <td style={{ ...td, textAlign: "right" }}>
                  {discAmt > 0 ? `${discAmt.toFixed(2)}` : "0.00"}
                </td>
                <td style={{ ...td, textAlign: "right" }}>
                  {isComposition ? "0.00" : `${taxRate}%`}
                </td>
                <td style={{ ...td, textAlign: "right" }}>
                  {isComposition ? "0.00" : taxAmt}
                </td>
          ...
          <tbody>
            ...
            {!isComposition && bill.tax > 0 && (
              <>
                <tr>
                  <td
                    style={{ ...td, border: "none", textAlign: "right" }}
                    colSpan={8}
                  >
                    CGST
                  </td>
                  <td style={{ ...td, border: "none", textAlign: "right" }}>
      ...
      <div style={{ display: "flex", borderBottom: "1px solid #999" }}>
        ...
        >
          ...
          <div
            style={{ fontSize: 11.5, color: "#444", whiteSpace: "pre-wrap" }}
          >
      ...
      <div style={{ display: "flex" }}>
        ...
        {(settings.showBankDetails || settings.showUpiQr) && (
          ...
          >
            ...
            {settings.showBankDetails && (
              <div>
                <div>
                  <strong>Name:</strong> {companies.legal_name || "—"}
                </div>
                <div>
        ...
        <div style={{ flex: 1, padding: "12px 16px", textAlign: "right" }}>
          <div style={{ marginBottom: 40, fontWeight: 600 }}>
            For {companies.legal_name || "Company Name"}
          </div>
